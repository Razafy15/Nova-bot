...
def trading_loop():
    ...
    while state["running"]:
        ...
        try:
            ...
            # Aza manao proposal vaovao raha misy trade mbola misokatra
            ...
            if check_strategy():
                ...
                # ============================================================
                # FANITSIANA 1:
                # Nisy "basis" tsy tao — ilaina TSY MAINTSY amin'ny Deriv API,
                # raha tsy izany dia error foana ny valiny (proposal tsy mety).
                #
                # FANITSIANA 2:
                # "duration_unit" novaina ho "s" (segondra) fa tsy "m" (minitra),
                # satria ny UI-nao dia "Duration (seconds)".
                # ============================================================
                proposal_payload = {
                    "proposal": 1,
                    "amount": state["stake"],
                ...
                # Miandry ny valiny "proposal" (mety ho haingana kokoa noho 3s,
                # fa tazomina ny 3s eto mba tsy hanova ny fitondran'ny loop hafa)
                time.sleep(3)
...
def on_message(socket, message):
    ...
    # ============================================================
    # FANITSIANA 3:
    # Ny filter taloha ("BOOM" in text or "R_" in symbol) dia nampiditra
    # ny Volatility Index rehetra (R_10, R_25, R_50, R_75, R_100) satria
    # samy manomboka amin'ny "R_" izy ireo. Ny symbol Boom marina dia
    # manomboka amin'ny "BOOM" ihany (BOOM300N, BOOM500, BOOM600,
    # BOOM900, BOOM1000) — koa io ihany no ekena.
    # ============================================================
    ...
    if msg_type == "proposal":
        ...
        # ============================================================
        # FANITSIANA 4:
        # Ny valiny "proposal" avy amin'i Deriv dia "ask_price", tsy
        # "price". Raha "price" no takina, dia 0 foana no azo, ka
        # mety ho lavina ny "buy" (price mismatch / invalid amount).
        # ============================================================
        ask_price = proposal.get("ask_price")
        if proposal_id and ask_price is not None:
            state["last_proposal"] = {
                "id": proposal_id,
                "price": ask_price
            }
            add_log(f"Proposal received: {proposal_id} - Ask price: {ask_price}")
        else:
            add_log("Proposal response missing id or ask_price.")
        return
    ...
    if msg_type == "buy":
        ...
        if contract_id:
            ...
            add_log("BUY ERROR: no contract_id in response.")
        return
